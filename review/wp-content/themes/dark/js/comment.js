...
WP_Dark_Comment.prototype = {
	...
	handleRequest: function(response) {
		...
		if((cmts.length - 1) == 0) {
			...
			$('rechts').getElementsByTagName('h2')[0].innerHTML = '1 Kommentar';
		} else {
			firstComment = $('rechts').getElementsByTagName('div')[1];
	...
	handleError: function(response) {
		...
		$A(this.form.getElementsByTagName('input')).each( function(el) { if (el.getAttribute('type') == 'submit') { el.setAttribute('value', 'Kommentar absenden'); }});
	}
}